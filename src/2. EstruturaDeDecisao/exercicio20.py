# Faça um Programa para leitura de três notas parciais de um aluno.
# O programa deve calcular a média alcançada por aluno e presentar:
# A mensagem "Aprovado", se a média for maior ou igual a 7,
# com a respectiva média alcançada;
# A mensagem "Reprovado", se a média for menor do que 7, com a
# respectiva média alcançada;
# A mensagem "Aprovado com Distinção", se a média for igual a 10.
p_nota = int(input("Digite sua primeira nota: "))
s_nota = int(input("Digite sua segunda nota: "))
t_nota = int(input("Digite sua terceira nota: "))
media = (p_nota + s_nota + t_nota) / 3

print(f"Sua media total é de: {media:.2f}")

if media >= 7.0 and media < 10:
    print("Aprovado!")
elif media < 7:
    print("Reprovado!")
elif media == 10:
    print("Aprovado com Distinção!")
else:
    print("Valores Incorretos")
